...
// The Checklist is done, let's fetch the events
	...
	$query = "select evid,severity,eventState,eventClass,prodstate,device,deviceClass,summary,component,trim(leading '|/' from systems) as systems,location,ownerid,count,date_format(from_unixtime(firsttime),'$date_format') as firsttime, date_format(from_unixtime(lasttime),'$date_format') as lasttime, round(unix_timestamp() - firsttime) as `timediff` from $cfg[db_tabl] where $cfg[ev_filt] and $context_clause $user_filter $evid_filter order by $cfg[ev_ordr] ;";
	$result = mysql_query($query);
		...
		$page['content'] .="\n\t\t\t<div id='container-$event[evid]' class='noclass'>
					...
					$page['content'] .= "
							<dl>
								...
								<dt>$msg[eventClass]</dt><dd>$event[eventClass]</dd>
								<dt>$msg[deviceClass]</dt><dd>$event[deviceClass]</dd>
							</dl>
					";
					$page['content'] .="
						</div>
