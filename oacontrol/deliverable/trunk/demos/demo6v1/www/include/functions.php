...
define( 'BASEIP', '10.1.7.211' );
define( 'FABIP', '192.168.100.210' );
define( 'SLOT3IP', '' );
define( 'SLOT4IP', '10.1.7.130' );
define( 'SLOT5IP', '10.1.7.129' );
define( 'SHMCIP', '10.1.7.201' );
...
//// this loop is where the comet long-poll happens
    ...
    while ( $loop_break == false) {
	    $snmp = getAllZRELinkState( FABIP,  'ZX1900FABRIC') ;
...
switch (  $slots[0][0]  ) {	//slot1
    case '"Switch"':
        ...
        $data["slot1"]["baseip"] = BASEIP ;		//
        $data["slot1"]["fabip"] = FABIP ;		//
        $data["slot1"]["M"] = $slots[0][1] ;	//mark M state
        $data["slot1"]["fabric ports"] = getAllZRELinkState( FABIP,  'ZX1900FABRIC') ; //load up on snmp data
        $data["slot1"]["base ports"] = getAllZRELinkState( BASEIP,  'ZX1900BASE') ; //load up on snmp data
        $data["slot1"]["fabric ext"] =  getExtState( FABIP,  'ZX1900FABRIC') ; 
        $data["slot1"]["fabric int"] =  getIntState( FABIP,  'ZX1900FABRIC') ; 
        $data["slot1"]["base ext"] =  getExtState( BASEIP,  'ZX1900BASE') ; 
        $data["slot1"]["base int"] =  getIntState( BASEIP,  'ZX1900BASE') ; 
        break;
...
switch (  $slots[2][0]  ) {	//slot3
    case '"Payload Switch"':
        ...
        $data["slot3"]["ip"] = SLOT3IP ;		//
        $data["slot3"]["M"] = $slots[2][1] ;	//mark M state
        $data["slot3"]["7300 ports"] = getAllZRELinkState( SLOT3IP,  'ZX7300') ; //load up on snmp data
        $data["slot3"]["7300 ext"] =  getExtState(SLOT3IP,  'ZX7300') ; 
        $data["slot3"]["7300 int"] =  getIntState( SLOT3IP,  'ZX7300') ; 
        break;
    case '"MPCBL0030"':
        $data["slot3"]["name"] = "SBC" ;
        $data["slot3"]["ip"] = SLOT3IP ;	
        $data["slot3"]["M"] = $slots[2][1] ;
        $data["slot3"][2] = getSNMPLinux( SLOT3IP,  'LINUXMIB') ;
        break;
     default:				// all other matches get marked empty!
...
switch (  $slots[3][0]  ) {	//slot4
    case '"Payload Switch"':
        ...
        $data["slot4"]["ip"] = SLOT4IP ;
        $data["slot4"]["7300 ports"] = getAllZRELinkState( SLOT4IP,  'ZX7300') ; //load up on snmp data
        $data["slot4"]["7300 ext"] =  getExtState(SLOT4IP,  'ZX7300') ; 
        $data["slot4"]["7300 int"] =  getIntState( SLOT4IP,  'ZX7300') ; 
        break;
    case '"MPCBL0030"':
        $data["slot4"]["name"] = "SBC" ;
        $data["slot4"]["ip"] = SLOT4IP ;
        $data["slot4"]["M"] = $slots[3][1] ;
        $data["slot4"][2] = getSNMPLinux( SHMCIP,  'LINUXMIB') ;
        break;
     default:
...
switch (  $slots[4][0]  ) {  //slot5
    case '"Payload Switch"':
        ...
        $data["slot5"]["ip"] = SLOT5IP ;
        $data["slot5"]["7300 ports"] = getAllZRELinkState( SLOT5IP,  'ZX7300') ; //load up on snmp data
        $data["slot5"]["7300 ext"] =  getExtState(SLOT5IP,  'ZX7300') ; 
        $data["slot5"]["7300 int"] =  getIntState( SLOT5IP,  'ZX7300') ; 
        break;
    case '"MPCBL0030"':
        $data["slot5"]["name"] = "SBC" ;
        $data["slot5"]["ip"] = SLOT5IP ;
        $data["slot5"]["M"] = $slots[4][1] ;
        $data["slot5"][2] = getSNMPLinux( SHMCIP,  'LINUXMIB') ;
        break;
     default:
...
$data["shmc"][0] = getSNMPShMC( SHMCIP,  'PPS-SENTRY-MIB') ;
$data["shmc"]["ip"] =  SHMCIP ;
