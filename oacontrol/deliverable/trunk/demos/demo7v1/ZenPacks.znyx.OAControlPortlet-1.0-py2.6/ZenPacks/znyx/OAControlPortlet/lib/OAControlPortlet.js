var OAControlPortletDatasource = Class.create();


OAControlPortletDatasource.prototype = {
    __class__ : "YAHOO.zenoss.portlet.OAControlPortletDatasource",
    __init__: function(settings) {
        this.baseLoc = settings.baseLoc;
    },
    get: function(callback) {
        this.callback = callback;
		//var url ;//= this.baseLoc; // baseLoc is the zenoss coporate site Datasource
        //if (url == YAHOO.zenoss.portlet.DEFAULT_SITEWINDOW_URL) {
           // url += '?v=' + YAHOO.zenoss.ZENOSS_VERSION + '&p=' + YAHOO.zenoss.ZENOSS_PRODUCT;
        //}
		var url = '/++resource++OAControlPortlet/oac.html'; // this '/++resource++OAControlPortletPortlet/test.html' is the "resources" dir in the zenpack 
...
YAHOO.zenoss.portlet.OAControlPortletDatasource = OAControlPortletDatasource;

var OAControlPortlet = YAHOO.zenoss.Subclass.create(
    YAHOO.zenoss.portlet.Portlet);
OAControlPortlet.prototype = {
    __class__: "YAHOO.zenoss.portlet.OAControlPortlet",
    __init__: function(args) {
        args = args || {};
        id = 'id' in args? args.id : getUID('OAControl');
        baseLoc = 'baseLoc' in args? args.baseLoc : YAHOO.zenoss.portlet.DEFAULT_SITEWINDOW_URL;
        bodyHeight = 'bodyHeight' in args? args.bodyHeight : 1000;
        title = 'title' in args? args.title: "OAControl Portlet";
        ...
        var datasource = 'datasource' in args? 
            args.datasource:
            new YAHOO.zenoss.portlet.OAControlPortletDatasource(
...
YAHOO.zenoss.portlet.OAControlPortlet = OAControlPortlet;
